...
void general_algorithm_abs_error(double v_0,double v_end,int n_steps)
{
  ...
  for (int i=0; i < abs_err.size(); i++){
      abs_err[i] = log10(fabs(u[i+1]-v[i+1]));
    }
...
int main()
{
  ...
  for (int i=0; i<n_steps_.size(); i++){
     general_algorithm_abs_error(0.0,0.0,n_steps_[i]);
  }
